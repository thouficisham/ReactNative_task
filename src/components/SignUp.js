...
const SignUp = () => {
    ...
    const handleSignUp = async () => {
        ...
        const isEmailvalidate = formInput.email.trim() === "" || !EMAIL_REGEX.test(formInput.email);
        const ispassWord = formInput.password.trim() === ""
        const checkIsUserValidation = userData.find((item) => item.email === formInput.email)
        ...
        if (valid) {
            ...
            await axios.post(url, formInput)
                .then((res) => Alert.alert("Registered"))
                .catch((err) => Alert.alert(err))
                ...
                <View style={styles.LoginContainer}>
                    <TouchableOpacity
                        ...
                        <Text style={styles.LoginText}>
                            Already have an account? {" "}<Text style={{color: '#82A3FF'}}>Login</Text> 
                        </Text>
                    </TouchableOpacity>
                </View>
            </View>
